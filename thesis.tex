...
%	We conclude with the discussion, in Chapter~\ref{chap:discussion}. 
	...
	\end{equation}
	This equivalent form of EIG first samples $\theta$ from the prior, and subsequently samples $y$ conditional on $\theta$. Thus, without performing Bayesian inference, we have one sample $\theta$ from the true posterior distribution, this is akin to root sampling \citep{guez2012efficient}.
	This trick is particularly helpful when we then replace the integrand with an estimate, such as in the `posterior' estimate of EIG \citep{foster2019variational}
	\begin{equation}
	\mathcal{I}(\xi) \ge \E_{p(\theta)}\left[\E_{p(y|\theta,\xi)}\left[ \log \frac{q_\phi(\theta|y,\xi)}{p(\theta)} \right]\right].
	\end{equation}
	which circumvents the need to perform explicit Bayesian inference.
	Variational and policy-based methods make extensive use of this trick \citep{foster2020unified,foster2021dad}, thereby tying them into using the prior predictive distribution as a simulator for $y$.
	...
	We investigated DAD with a number of steps up to about $T=30$ and found that it performed well. However, one can imagine potential problems with this approach as $T$ grows to be very large.
	...
	The estimate of marginal likelihood used in the sPCE objective, namely $\frac{1}{L+1}\sum_{\ell=0}^L p(h_T|\theta_\ell,\pi)$, is not much different from an importance sampling estimate using the prior, although with the distinction of using the true sample $\theta_0$ in the estimate.
